...
bool FFurComb::LineTraceComponent(struct FHitResult& OutHit, const FVector Start, const FVector End, const struct FCollisionQueryParams& Params, UGFurComponent* FurComponent) const
{
	...
	if (bHitBounds || bInsideBounds)
	{
		...
		else
		{
			...
			{
				// Compute the normal of the triangle
				const FVector& P0 = VertexBuffer.VertexPosition(Indices[(TriangleIndex * 3) + 0]);
				const FVector& P1 = VertexBuffer.VertexPosition(Indices[(TriangleIndex * 3) + 1]);
				const FVector& P2 = VertexBuffer.VertexPosition(Indices[(TriangleIndex * 3) + 2]);
