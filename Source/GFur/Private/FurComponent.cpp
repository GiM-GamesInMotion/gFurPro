...
//	if (FeatureLevel >= ERHIFeatureLevel::ES3_1)
	{
		...
		if (SkeletalGrowMesh && SkeletalGrowMesh->GetResourceForRendering())
		{
			...
			auto NumLods = SkeletalGrowMesh->GetResourceForRendering()->LODModels.Num();
...
void UGFurComponent::updateFur()
{
	...
	if (SkeletalGrowMesh)
	{
		...
		const auto& LOD = SkeletalGrowMesh->GetResourceForRendering()->LODModels[Scene->GetCurrentMeshLodLevel()];
	...
	uint32 RevisionNumber = 2;// MasterPoseComponent.IsValid() ? MasterPoseComponent->GetBoneTransformRevisionNumber() : 0;TODO
...
void UGFurComponent::UpdateFur_RenderThread(FRHICommandListImmediate& RHICmdList, bool Discontinuous)
{
	...
	if (FurProxy)
	{
		...
		if (SkeletalGrowMesh)
		{
			const auto& LOD = SkeletalGrowMesh->GetResourceForRendering()->LODModels[FurProxy->GetCurrentMeshLodLevel()];
...
void UGFurComponent::CreateMorphRemapTable(int32 InLod)
{
	...
	const auto& MasterLodModel = MasterMesh->LODModels[InLod];
	TArray<FSoftSkinVertex> MasterVertices;
	MasterLodModel.GetVertices(MasterVertices);

	auto* Mesh = SkeletalGrowMesh->GetResourceForRendering();
	const auto& LodModel = Mesh->LODModels[InLod];
	TArray<FSoftSkinVertex> Vertices;
	LodModel.GetVertices(Vertices);

	MorphRemapTable.AddUninitialized(MasterVertices.Num());

	bool HasExtraBoneInfluences = MasterLodModel.DoesVertexBufferHaveExtraBoneInfluences() && LodModel.DoesVertexBufferHaveExtraBoneInfluences();
	uint32 NumBoneInfluences = HasExtraBoneInfluences ? MAX_TOTAL_INFLUENCES : MAX_INFLUENCES_PER_STREAM;
	uint32 UVCount = FMath::Min(MasterLodModel.NumTexCoords, LodModel.NumTexCoords);

	for (const auto& MasterSection : MasterLodModel.Sections)
	{
		for (const auto& Section : LodModel.Sections)
		{
			if (MasterSection.MaterialIndex == Section.MaterialIndex)
			{
				for (uint32 i = MasterSection.BaseVertexIndex; i < MasterSection.BaseVertexIndex + MasterSection.NumVertices; i++)
				{
					...
					{
						if (MasterVertices[i].Position == Vertices[j].Position
							&& MasterVertices[i].TangentX == Vertices[j].TangentX
							&& MasterVertices[i].TangentY == Vertices[j].TangentY
							&& MasterVertices[i].TangentZ == Vertices[j].TangentZ)
						{
							bool equals = true;
							for (uint32 k = 0; k < NumBoneInfluences; k++)
							{
								if (MasterVertices[i].InfluenceBones[k] != Vertices[j].InfluenceBones[k] || MasterVertices[i].InfluenceWeights[k] != Vertices[j].InfluenceWeights[k])
								{
									equals = false;
									break;
								}
							...
							{
								if (MasterVertices[i].UVs[k] != Vertices[j].UVs[k])
								{
									equals = false;
									break;
								}
